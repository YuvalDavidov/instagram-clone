        ...
        <article v-if="editSection.editPass" class="edit-password-article">
            <h2>Edit Password</h2>
                ...
                <form @submit.prevent="submitPassword" class="edit-password-form">
                    <label for="curr-password"><span>Current Password</span> <input v-model="password.currPassword" type="password" id="curr-password"
                            placeholder="Current Password"></label>
                    <label for="new-password"><span>New Password</span> <input v-model="password.newPassword" type="password" id="new-password"
                            placeholder="New Password"></label>
                    <label for="confirm-password"><span>Confirm new Password</span> <input @click="() => {this.error.password = false}" v-model="password.confirmNewPassword" type="password" id="confirm-password"
                            placeholder="Confirm your Password"></label>
                            <label v-if="error.password"><span></span><span class="password-error">Passwords don't match!</span></label>
                            <label><span></span><button :disabled="error.password" type="submit" class="submit-btn">Submit</button></label>
                </form>
            </section>
        </article>
            ...
            <section class="edit-profile-actions">
                <div class="user-details">
                    ...
                    <div>
                        ...
                        <input @change="onUploadImg" ref="fileInput" type="file" class="change-photo-input">
                        <button @click="selectFile">Change profile photo</button>
                    </div>
                </div>
                <form class="edit-profile-form" @submit.prevent="submit">
                    <label for="website"><span>Website</span> <input @change="checkIfDetailsChanged" v-model="user.website" type="text" id="website"
                            placeholder="Website"></label>

                    <label class="bio" for="bio"><span>Bio</span> <textarea @change="checkIfDetailsChanged" v-model="user.bio" name="" id="bio" cols="30"
                    ...
                    <label><span></span><button :disabled="error.profileDetailsNotChanged" type="submit" class="submit-btn">Submit</button></label>
                </form>
            </section>
        </article>
    </section>
...
export default {
    ...
    methods: {
        ...
        async submitPassword() {
            ...
            if (this.password.newPassword !== this.password.confirmNewPassword) this.error.password = false
       ...
       async onUploadImg(ev) {
        ...
        checkIfDetailsChanged() {
            this.error.profileDetailsNotChanged = (this.user.website !== this.$store.getters.GetUser.website) || 
            (this.user.bio !== this.$store.getters.GetUser.bio) ? false : true;
